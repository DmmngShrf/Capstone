<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="form">
        ...
        <div class="flex-row">
            <div>
            <input type="checkbox" id="remember_me" name="remember" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" style="color: black;"><label for="remember_me">{{ __('Remember me') }}</label>
            </div>
            <span class="span">
                @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="underline text-sm text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" style="color: black;">{{ __('Forgot password?') }}</a>
                @endif
            </span>
        </div>

        <button type="submit" class="button-submit" href="{{ route('register') }}">{{ __('Sign In') }}</button>
